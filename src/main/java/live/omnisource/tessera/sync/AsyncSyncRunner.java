...
/**
 * Async wrapper for FeatureSyncService. @Async requires a separate bean (Spring proxies can't
 * intercept self-calls). The sync thread pool is configured via Spring's TaskExecutor.
 */
@Slf4j
@Service
  ...
  public void run(String workspace, String datastore, String layer) {
    try {
      ...
      if ("FAILED".equals(result.status())) {
        ...
        log.info(
            "Async sync completed for {}/{}/{}: {} features in {}s",
            workspace,
            datastore,
            layer,
      ...
      log.error(
          "Unexpected error in async sync for {}/{}/{}: {}",
          workspace,
          datastore,
          layer,
          e.getMessage(),
