 ...
 *
 * <p>Uses raw JDBC batch inserts (not JPA) for throughput. Geometry is written as WKB via
 * ST_GeomFromWKB with SRID 4326. H3 indexing is computed server-side using the PostgreSQL h3
 * extension.
 */
@Slf4j
@Component
public class FeatureBatchWriter {
  ...
  /**
   * H3 index insert: compute the h3 cell from a lat/lng centroid at the given resolution. Uses the
   * PG h3 extension's h3_lat_lng_to_cell function.
   */
  ...
  public int writeBatch(
      UUID sourceId, String sourceTable, List<ExtractedFeature> features, int[] h3Resolutions) {
  ...
  private void publishIngestEvent(
      UUID sourceId, String sourceTable, List<ExtractedFeature> features) {
    try {
      ...
      eventPublisher.publishEvent(
          new FeatureIngestEvent(sourceId, sourceTable, features.size(), envelope, now, now));
    } catch (Exception e) {
      log.warn("Failed to publish ingest event: {}", e.getMessage());
    }
  }

  private void indexH3Batch(
      List<ExtractedFeature> features,
      long[] featureIds,
      Timestamp[] ingestTimes,
      int[] resolutions) {
