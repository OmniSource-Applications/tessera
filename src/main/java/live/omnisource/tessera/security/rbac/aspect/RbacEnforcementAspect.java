...
public class RbacEnforcementAspect {
  ...
  @Around(
      "@annotation(live.omnisource.tessera.security.rbac.annotations.RequireGlobalPermission) || "
  ...
  @Around(
      "@annotation(live.omnisource.tessera.security.rbac.annotations.RequireWorkspacePermission) || "
  ...
  public Object enforceWorkspace(ProceedingJoinPoint pjp) throws Throwable {
    ...
    final RequireWorkspacePermission ann =
        findWorkspaceAnnotation(method, pjp.getTarget().getClass());
    if (ann == null) return pjp.proceed();
    ...
    if (!authz.hasWorkspacePermission(a, workspace, ann.value())) {
      throw new AccessDeniedException(
          "Missing permission: " + ann.value() + " for workspace=" + workspace);
    }
