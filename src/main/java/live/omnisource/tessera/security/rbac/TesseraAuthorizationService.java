...
public class TesseraAuthorizationService {
  ...
  public TesseraAuthorizationService(
      RbacPolicyStore store, TesseraSubjectResolver subjectResolver) {
    this.store = store;
  ...
  public boolean hasWorkspacePermission(
      Authentication auth, String workspace, TesseraPermission permission) {
    refreshIfNeeded();
    if (permission == null) return false;
  ...
  private Set<TesseraPermission> rolesToPermissions(Set<TesseraRole> roles) {
    ...
    for (TesseraRole r : roles) {
      out.addAll(
          BuiltInRolePermissions.ROLE_PERMISSIONS.getOrDefault(
              r, EnumSet.noneOf(TesseraPermission.class)));
    }
    return out;
  }
