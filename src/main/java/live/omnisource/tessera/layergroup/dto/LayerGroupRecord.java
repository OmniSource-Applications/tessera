...
/**
 * Persisted metadata for a layer group. Written to {@code
 * data_dir/etc/catalog/layergroups/{name}/group.json}
 */
...
public record LayerGroupRecord(
  ...
  public LayerGroupRecord withMembers(List<LayerGroupMember> members) {
    return new LayerGroupRecord(
        name, description, active, List.copyOf(members), createdAt, Instant.now());
  }
  ...
  /**
   * Collect the source_id UUIDs from all members as a string array. Used to build the
   * ANY(?::uuid[]) parameter for geo_features queries.
   */
  public String[] sourceIds() {
    return members.stream().map(LayerGroupMember::sourceId).distinct().toArray(String[]::new);
  }
}
