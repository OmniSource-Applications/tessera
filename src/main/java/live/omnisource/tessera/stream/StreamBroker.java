...
public class StreamBroker {
  ...
  public void subscribe(
      StreamSubscription subscription, Consumer<List<Map<String, Object>>> callback) {
    subscriptions.put(subscription.id(), new ActiveSub(subscription, callback));
    log.info(
        "Stream subscription registered: id={} protocol={} sourceId={} bbox={}",
        subscription.id(),
  ...
  public void unsubscribe(String subscriptionId) {
    ...
    if (removed != null) {
      ...
      log.info(
          "Stream subscription removed: id={} delivered={}",
          subscriptionId,
          removed.subscription.deliveredCount());
    }
  }
  ...
  public void onFeatureIngested(FeatureIngestEvent event) {
    ...
    log.debug(
        "Ingest event: source={} table={} count={} envelope={}",
        event.sourceId(),
        event.sourceTable(),
        event.featureCount(),
        event.envelope());
  ...
  private void deleiverToSubscription(ActiveSub active, FeatureIngestEvent event) {
    ...
    log.debug(
        "Delivered {} features to subscription {} (cursor now {})",
        features.size(),
        subscription.id(),
        maxUpdatedAt);
  }

  private List<Map<String, Object>> queryFeaturesSince(StreamSubscription sub) {
    ...
    if (sub.spatialFilter() != null) {
      sql.append(
          " AND ST_Intersects(f.geometry, ST_MakeEnvelope(:minX, :minY, :maxX, :maxY, 4326))");
