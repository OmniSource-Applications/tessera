...
/**
 * KeyProvider is a utility class responsible for managing and decoding cryptographic keys. It uses
 * a base64-encoded string, referred to as a "salt," to derive a secure key. This class validates
 * the salt and ensures that the resulting key meets the expected length for cryptographic
 * operations. Instances of this class can be created using the {@link
 * #fromProperties(ApplicationProperties.CryptoProperties)} method. The class performs validation
 * during initialization to ensure that a valid salt is provided.
 */
