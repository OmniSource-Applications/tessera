...
/**
 * Persisted metadata for a single layer within a data store. Written to
 * {workspace}/data/{datastore}/layers/{layer}/layer.json
 */
...
public record LayerRecord(
   ...
   * @param orderByColumn column to ORDER BY for incremental reads (e.g. "updated_at", "id"). null →
   *     full rescan with hash-based dedup each poll.
   */
  public record SyncConfig(boolean enabled, int pollIntervalSeconds, String orderByColumn) {
  ...
  public static LayerRecord fromIntrospection(
      LayerDto dto, IntrospectionResult.SpatialTable table) {
