...
public final class CassandraConnector implements DataSourceConnector {
  ...
  public List<String> listSchemas(String secretRefKey) {
    return sessionFor(secretRefKey).getMetadata().getKeyspaces().values().stream()
  ...
  public List<String> listTables(String secretRefKey, String schema) {
    return sessionFor(secretRefKey)
        .getMetadata()
        .getKeyspace(schema)
        .map(
            ks ->
                ks.getTables().values().stream()
  ...
  public SchemaMetadata introspectTable(String secretRefKey, String schema, String table) {
    ...
    var cqlTable =
        keyspace
            .getTable(table)
            .orElseThrow(
                () -> new IllegalArgumentException("Table not found: " + schema + "." + table));

    var pkNames =
        cqlTable.getPrimaryKey().stream()
            .map(col -> col.getName().asInternal())
            .collect(java.util.stream.Collectors.toSet());

    var columns =
        cqlTable.getColumns().values().stream()
            .map(
                col -> {
                  ...
                  return new ColumnMetadata(
                      name, cqlType, cqlType, true, pkNames.contains(name), isGeo);
                })
            .toList();
  ...
  public Stream<RawRecord> streamTable(
    ...
    var stmt =
        SimpleStatement.builder("SELECT * FROM " + schema + "." + table)
