  ...
  public CassandraSessionFactory(
    ...
    this.sessions =
        CacheBuilder.newBuilder()
            ...
            .expireAfterAccess(
                applicationProperties.connectors().cassandra().expirationMinutes(),
                TimeUnit.MINUTES)
            .removalListener(
                (RemovalListener<String, CqlSession>)
                    notification -> {
  ...
  private CqlSession build(String secretRefKey) {
    try {
      ...
      CqlSessionBuilder builder =
          CqlSession.builder()
              .withLocalDatacenter(
                  credentials.datacenter() != null ? credentials.datacenter() : DEFAULT_DATACENTER)
              .withAuthCredentials(credentials.username(), credentials.password())
              .withKeyspace(credentials.keyspace());
