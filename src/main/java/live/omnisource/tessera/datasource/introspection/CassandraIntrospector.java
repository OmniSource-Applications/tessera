 ...
 *
 * <p>Cassandra has no native spatial types. This introspector scans keyspace tables for
 * recognizable lat/lng column pairs (double type) and reports them as POINT geometry tables.
 *
 * <p>Recognized latitude column names: lat, latitude, y, lat_y Recognized longitude column names:
 * lng, lon, longitude, x, lng_x, lon_x
 *
 * <p>Usage from the Cassandra connector: @Override public IntrospectionResult introspect(String
 * secretRefKey) { try (CqlSession session = sessionFor(secretRefKey)) { return
 * CassandraIntrospector.introspect(session, keyspace); } }
 */
@Slf4j
public final class CassandraIntrospector {
  ...
  private static final Set<String> LAT_NAMES = Set.of("lat", "latitude", "y", "lat_y");

  private static final Set<String> LNG_NAMES =
      Set.of("lng", "lon", "longitude", "x", "lng_x", "lon_x");
  ...
  public static IntrospectionResult introspect(CqlSession session, String keyspace) {
    ...
    log.info(
        "Cassandra introspection: discovered {} spatial tables in keyspace '{}'",
        tables.size(),
        keyspace);
  ...
  private static double[] fetchExtent(
      CqlSession session, String keyspace, String table, String latCol, String lngCol) {
    try {
      String cql =
          String.format(
