...
public class ExternalDataSourceFactory {
                ...
                (RemovalListener<String, HikariDataSource>)
                    notification -> {
                      ...
                      if (ds != null && !ds.isClosed()) {
                        ds.close();
                        log.info(
                            "Evicted connection pool for: {} (reason: {})",
                            notification.getKey(),
  ...
  /**
   * Resolve a DataSource for an external source whose credentials are stored under the given key.
   *
  ...
  /**
   * Store credentials for a new external source into the SecureFileStore, returning the SecretRef
   * that identifies it.
   */
